...
const Title = () => {
    const { spaceNum } = useContext(Context)
        ...
        `}>
            Мой {
                text.split('').map((letter, ind) => <span key={ind}
                    style={{ backgroundImage: (bgIndexed >= ind && `url(spaces/space${spaceNum}_img/space${spaceNum}_lg.jpg)`), backgroundPositionX: `${ind*2}rem` }}
